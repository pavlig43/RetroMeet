...
fun ResumeScreen(
    ...
    when (val state = resumeState) {
        ...
        is ResumeState.Success -> ResumeScreenPrivate(
            ...
            updateResume = { viewModel.updateResume { returnMainScreen() } },
            modifier = modifier,
        )
    }
...
private fun ResumeScreenPrivate(
    ...
    Column(
        ...
        val multiEnumList: Map<List<EnumUi>, List<EnumUi>> = mapOf(
            PetUi.entries.toList() to resumeUi.pets  ,
            MusicGenreUi.entries.toList() to resumeUi.favoriteMusicGenres
        )
