...
class ResumeViewModel @Inject constructor(
    ...
    private val loginId = checkNotNull(saveStateHandle.toRoute<ResumeRoute>()).loginId

    private val _resumeState = MutableStateFlow<ResumeState>(ResumeState.Loading)
    val resumeState = _resumeState.asStateFlow()

    private val _resumeUi = MutableStateFlow(ResumeUi(loginId))
    ...
    private suspend fun getResume() {
        val start: ResumeState =
            userInfoRepository.getUserInfo(loginId).mapTo { it.toResumeUi() }.toResumeState()
    ...
    fun updateResume(returnToMainScreen: (Int) -> Unit) {
        viewModelScope.launch {
            userInfoRepository.updateUserInfo(_resumeUi.value.toUserInfoUpdateRequest())
            returnToMainScreen(loginId)
        }
    }
...
private fun UserInfoResponse.toResumeUi(): ResumeUi {
    return ResumeUi(
        loginId = loginId,
        city = city ?: "",
...
private fun ResumeUi.toUserInfoUpdateRequest(): UserInfoUpdateRequest {
